...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * first insertion（直到第一次插入的时候，表会初始化为2的倍数）.
     * Each bin in the table normally contains a
     ...
     */
    public ConcurrentHashMap(int initialCapacity,
                             float loadFactor, int concurrencyLevel) {   // concurrencyLevel，表示能够同时更新ConccurentHashMap且不产生锁竞争的最大线程数。默认值为16
    ...
    static final class TreeBin<K,V> extends Node<K,V> {
        ...
        static <K,V> TreeNode<K,V> balanceInsertion(TreeNode<K,V> root,
            ...
            for (TreeNode<K,V> xp, xpp, xppl, xppr;;) {
                if ((xp = x.parent) == null) {  // 如果x为根节点，直接返回
                    x.red = false;
                    return x;
                }
