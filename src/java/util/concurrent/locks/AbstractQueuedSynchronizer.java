...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        /**
         * 与AQS的独占功能不同，当锁被头节点获取后，独占功能是只有头节点获取锁，其余节点的线程继续沉睡，等待锁被释放后，才会唤醒下一个节点的线程，
         * 而共享功能是只要头节点获取锁成功，就在唤醒自身节点对应的线程的同时，继续唤醒AQS队列中的下一个节点的线程，
            ...
            if (h != null && h != tail) {
                ...
                if (ws == Node.SIGNAL) {
                    ...
                    unparkSuccessor(h);
                         ...
                         !compareAndSetWaitStatus(h, 0, Node.PROPAGATE))   //如果本身头节点的waitStatus是出于重置状态（waitStatus==0）的，将其设置为“传播”状态。 意味着需要将状态向后一个节点传播。
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
         ...
         *   static final int PROPAGATE = -3;
         */
        if (ws == Node.SIGNAL)  //只有当前节点的前一个节点为SIGNAL时，当前节点才能被挂起。
            /*
    ...
    private void doAcquireShared(int arg) {
        final Node node = addWaiter(Node.SHARED);//将当前线程以共享模式加入同步等待队列。
        boolean failed = true;
        try {
            boolean interrupted = false;
            for (;;) {
                ...
                if (p == head) {
                    int r = tryAcquireShared(arg);//如果p是头节点。再次调用tryAcquireShared方法。
                    if (r >= 0) {  //如果tryAcquireShared方法执行成功，执行setHeadAndPropagate
    ...
    public final void acquireSharedInterruptibly(int arg)
        ...
        if (Thread.interrupted())
            throw new InterruptedException();
        if (tryAcquireShared(arg) < 0)  //获取共享锁，小于0，表示获取失败,放进队列
