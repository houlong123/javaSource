 ...
 * elements (including {@code null}). 双链表实现了List和Deque接口。实现了列表的所有可选操作，可以包含所有包含null的元素
 *
 ...
 *
 * 如果迭代器被创建之后，列表的结构被修改了，除了通过迭代器自己的remove()和add()方法外，迭代器将会抛出ConcurrentModificationException异常
 *
